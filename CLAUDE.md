...
**Selective health** (`MONITOR_SELECTIVE_HEALTH=true`, used when full `/health` is off): the full `/health` actively pings every backend, which wakes KServe Serverless (scale-to-zero) backends — that's why prod sets `MONITOR_HEALTH=false`. The selective loop (`_selective_health_loop`) instead reads the latest snapshot, picks only models whose **every** deployment is positively safe to ping (`select_health_check_models` in `services/litellm.py`: KServe RawDeployment or plain Service; never Serverless/scale-to-zero/knative-pa/external/undetermined — fail-safe), and calls `/health?model=<name>` per model in parallel (`fetch_health_for_models`). The aggregated result is health-shaped and reuses the same `_health` injection path, so checked models get UP/DOWN (`status_source=health`) and skipped ones keep the k8s fallback (`?`). Per-model manual override: LiteLLM `model_info.active_health_check` (false=always skip; true=allow undetermined/external, but cannot override a positive Serverless/scale-to-zero signal). Full `/health` (`MONITOR_HEALTH=true`) takes precedence — the selective loop then doesn't start.
