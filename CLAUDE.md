...
### Dashboard tabs — `상태` / `부하` (web only, v1.3.0)
The page is split into two panes by the question they answer: **상태** ("떠 있나" — LiteLLM `/health` + k8s replicas/GPU, 5s) and **부하** ("지금 바쁜가" — backend Pod `/metrics`, 60s). Before this, both lived in one column of headline cards, which reached **10 cards** and mixed 장애 with 혼잡; it is now 6 + 4.

**It splits the view, not the data.** There is still one snapshot and one poll: `render()` draws **both panes every cycle** and switching tabs only flips `[hidden]` — no re-fetch, no re-render. That is what keeps `/snapshot.html` working off the same single template (graph coordinates are all computed in JS, so drawing into a hidden pane is safe). Do not "optimize" this into rendering only the active pane — the 부하 탭's summary dot (worst load state, shown next to the tab name) has to stay live while the 상태 탭 is open, or splitting the view hides the signal it was meant to surface.

The 부하 탭 owns everything load: its own cards, its own table, its own search + `등급` filter, and the `⟳ 부하` button + `N초 전` label (which used to sit in the 상태 탭's filter row, pushing it to 9 controls). Each tab keeps a **separate** search box — they are different questions, not two filters on one table — and `/`·`Esc` act on the tab you are looking at (`curSearch()`). Selected tab and both search boxes persist in `sessionStorage` (`mm_filters`).

`curTab` is the **chosen** tab; `effTab()` is what actually renders. When the 부하 탭 does not exist (`MONITOR_LOAD=false`, or per-user `load=off`) the choice is preserved but the 상태 탭 is shown — overwriting `curTab` would wipe the restore before the first snapshot arrives (the tab starts hidden). `clearLoadPane()` blanks the pane and hides the tab on the fail-closed paths (`showNeedKey`/`showUvError`) so a previous admin view's numbers never linger.

The 부하 표 sorts **worst grade first** (then most-queued, then name) — deliberately unlike the 상태 표's fixed name order: one is a list, the other is triage. Group rows roll up per `model_name`: numbers are sums (`sumLoad`) but the grade is `compositeLoad` (routing-dependent — least-busy picks the *idlest* backend), so `표본` must also be a sum across backends (`sumPods`) or the evidence column contradicts the numbers beside it. `TOK/S` is dropped in per-user views because `user_access` never flattens `throughput` — a column of `-` for every row only makes "0 or unknown?" harder. Everywhere: a missing value renders `-`, never `0`.
...
tok/s · 등급(idle/ok/BUSY/FULL)을 deployment 행에 붙인다(화면에서는 **부하 탭** — 위 절). Pod 주소는 `gpu.collect_gpu_for_service`
...
- per-user 뷰에는 평탄한 스칼라(`load_state`/`load_running`/...)로만 나간다 — `per_pod` 에 Pod 주소가
  ...
  **노출 단계는 설정**(`MONITOR_USER_VIEW_LOAD` / `user_view.load`): `off`(키 자체 없음 + 최상위
  ...
  (`_slim_load`). `off` 면 대시보드의 **부하 탭 자체가 사라진다**(빈 탭을 남기지 않는다). `summarize` 의 **`load_state_known` 은 `load_known`(수치)과 분리**돼 있다 —
  summary 모드는 수치가 없어 한 플래그로 묶으면 등급 카드까지 사라진다. `⟳ 부하`(수동 갱신)는
  백엔드 팬아웃이라 admin 전용이고 비-admin 화면에서는 버튼 자체를 숨긴다. 버튼과 나이 표시는
  부하 탭의 탭바에 있다(v1.3.0 전에는 상태 표 필터 줄에 끼어 있었다).
