...
> **TYPE 2축 분리 (v0.5.10)** — 기존 단일 TYPE(vllm/sglang/kserve 혼합, 첫 매칭이 나머지를
...
- **라우팅 그래프 (v0.5.10)**: per-user 뷰에서도 Model↔Backend 그래프를 **항상** 그립니다.
  Service 이름은 숨긴 채 **익명 식별자 `backend_ref`**(솔트 해시 8자)로 노드/공유(⇄) 토폴로지만
  유지하므로 내부 구조가 노출되지 않습니다(목록이 단순해도 그래프 표시).
- **내 뷰 JSON (v0.5.10, 디버그용)**: 일반 키로 조회 중일 때 상단의 **`🐞 내 뷰 JSON`** 버튼으로
