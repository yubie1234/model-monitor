...
> 멀리 떨어져 있어도 무엇과 이어졌는지 바로 보입니다(간선도 모인 위치로 다시 그림). **노드를 클릭하면 그 배치를 고정(pin)해 얼려두어**, 끌어모은 노드 위로 마우스를 올려 값(이름·replicas·GPU tooltip)을 읽어도 배치가 흐트러지지 않습니다(v0.5.6 — 예전엔 hover 가 고정을 가로채 풀리는 것처럼 보였음). 고정은 5초 자동 갱신·필터 변경·브라우저 새로고침에도 유지됩니다(v0.5.9 — 배경 클릭/재클릭으로만 해제, 탭 닫으면 소멸). 헤드라인 **Replicas** 합계는 `(namespace, service)` 기준으로 **dedup** 하므로
